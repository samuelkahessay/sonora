...
final class AnalysisRepositoryImpl: ObservableObject, AnalysisRepository {
    ...
    private func validateEnvelopeJSON(data: Data, expectedMode: AnalysisMode) -> Bool {
        ...
        case .distillSummary:
            ...
            return true
        case .distillActions:
            guard payload["action_items"] is [Any] else { return false }
            return true
        case .distillThemes:
            guard payload["key_themes"] is [Any] else { return false }
            return true
        case .distillReflection:
            guard payload["reflection_questions"] is [Any] else { return false }
            return true
        case .themes:
