...
final class DuplicateHeuristicsTests: XCTestCase {
    func testTitleMatchWithinWindowSameDay() {
        ...
        let target = EventsData.DetectedEvent(
            id: "1", title: "Design Sync", startDate: now, endDate: now.addingTimeInterval(3600), location: nil, participants: nil, confidence: 0.9, sourceText: "discuss design", memoId: nil
        )
    ...
    func testSourceMatchWithinWindowSameDay() {
        let now = Date(timeIntervalSince1970: 1_700_000_000)
        let target = EventsData.DetectedEvent(
            id: "1", title: "Weekly", startDate: now, endDate: now.addingTimeInterval(3600), location: nil, participants: nil, confidence: 0.9, sourceText: "plan roadmap", memoId: nil
        )
    ...
    func testOutsideWindowOrDifferentDayIsNotDuplicate() {
        let now = Date(timeIntervalSince1970: 1_700_000_000)
        let target = EventsData.DetectedEvent(
            id: "1", title: "Design Sync", startDate: now, endDate: now.addingTimeInterval(3600), location: nil, participants: nil, confidence: 0.9, sourceText: "discuss", memoId: nil
        )
        ...
        let nextDay = DuplicateHeuristics.SimpleEvent(title: "design sync", startDate: Calendar.current.date(byAdding: .day, value: 1, to: now)!, endDate: Calendar.current.date(byAdding: .day, value: 1, to: now)!.addingTimeInterval(3600), isAllDay: false, notes: nil)
