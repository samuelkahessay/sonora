...
final class AdaptiveThresholdPolicyTests: XCTestCase {
    func testShortSimpleContentRaisesThresholds() {
        let ctx = DetectionContext(
            ...
            transcriptLength: 40,
            sentenceCount: 1,
            hasDatesOrTimes: false,
            hasCalendarPhrases: false,
            hasRelativeDatePhrases: false,
            hasWeekendReferences: false,
            imperativeVerbDensity: 0.0,
        ...
        XCTAssertGreaterThanOrEqual(t.reminder, 0.70)
    }
    ...
    func testCalendarSignalsLowerReminderThreshold() {
        let ctx = DetectionContext(
            ...
            hasCalendarPhrases: true,
            hasRelativeDatePhrases: true,
            hasWeekendReferences: true,
            imperativeVerbDensity: 0.03,
            localeIdentifier: "en_US",
            avgSentenceLength: 60
        )
        let policy = DefaultAdaptiveThresholdPolicy()
        let t = policy.thresholds(for: ctx)
